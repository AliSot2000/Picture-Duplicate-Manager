...
class PhotoAPI:
    ...
    def _get_best_match_type_common(self, tgt_fp: str, file_hash: str, match_keys: List[int]) \
        ...
        for m_key in match_keys:
            ...
            if m_newest_hash == file_hash and not binary_match:
                ...
                self.rare_occurrence_logger_name.warning("Found files different hashes but match binary.")
    ...
    def modify_timezone(self,
        ...
        if rename:
            ...
            # INFO: Updates the key_to_filepath_cache
            self._internal_move_file(new_datetime=new_dt, key=key, flags=flags, dt=dt, db_local_dir=db_local_dir,
                                     dbn=db_name)
    ...
    def move_to_duplicates(self, child_key: int, parent_key: int, copy_google_metadata: bool = True):
        ...
        child_data = self.db.get_replace_data(child_key)
        ...
        children = self.db.list_children(child_key)
        count = len(children)
        if count > 0:
        ...
        self.db.migrate_parent_duplicate(child_key=child_key, parent_key=parent_key, known=True)

        # Marking row as duplicate in metadata table
        self.db.update_row_metadata_table(key=child_key, replaced=MediaType.DUPLICATE)
