...
class PhotoDB(BaseSQliteDB):
    ...
    def add_caches(self, object_cache_size: int = 1024, int_cache_size: int = 16384):
        ...
        """
        self.view_header_cache = Cache(object_cache_size)
        self.view_row_cache = Cache(object_cache_size)
        self.view_key_cache = Cache(int_cache_size)

    def clear_caches(self):
        """
        Clear all caches of the views.
        """
        if self.view_key_cache is not None:
            self.view_key_cache.reset()
        if self.view_row_cache is not None:
            self.view_row_cache.reset()
        if self.view_header_cache is not None:
            self.view_header_cache.reset()
    ...
    def mark_import_table_as_stale(self, name: str = None):
        """
        ...
        :param name: name of import table to mark as stale
        """
        if name is None:
            # The stale flags is bit 1,
            # And we only update the flags by adding a +1 if that flag hasn't already been set mod(flags, 2) == 0
            stmt = "UPDATE import_table SET flags = flags + 1 WHERE table_name = ? AND mod(flags, 2) == 0"
            args = (name,)
        else:
    ...
    def set_imported_status(self, tbl_name: str, key: int, status: ImportStatus, import_key: int = None):
        ...
        if status == ImportStatus.IGNORE:
            ...
            self.debug_execute(stmt=f"UPDATE `{tbl_name}` SET imported = ?,  import_key = ? "
                               ...
                               args=(ImportStatus.IMPORTED.value, import_key, key))
            assert self.sq_cur.rowcount == 1, \
